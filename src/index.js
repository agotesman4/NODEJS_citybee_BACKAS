...
app.get("/vehicles/lt", async (req, res) => {
  try {
    const con = await mysql.createConnection(mysqlConfig);
    const [data] = await con.execute(
      `SELECT vehicles.id, models.name, vehicles.number_plate, models.hour_price + modes.hour_price * 0.21 AS price_pvm, vehicles.country_location FROM vehicles INNER JOIN models ON vehicles.model_id = models.id AND vehicles.country_location = "LT"`
    );
    con.end();
...
app.get("/vehicles/lv", async (req, res) => {
  try {
    const con = await mysql.createConnection(mysqlConfig);
    const [data] = await con.execute(
      `SELECT vehicles.id, models.name, vehicles.number_plate, models.hour_price + modes.hour_price * 0.21 AS price_pvm, vehicles.country_location FROM vehicles INNER JOIN models ON vehicles.model_id = models.id AND vehicles.country_location = "LV"`
    );
    con.end();
...
app.get("/vehicles/ee", async (req, res) => {
  try {
    const con = await mysql.createConnection(mysqlConfig);
    const [data] = await con.execute(
      `SELECT vehicles.id, models.name, vehicles.number_plate, models.hour_price + modes.hour_price * 0.21 AS price_pvm, vehicles.country_location FROM vehicles INNER JOIN models ON vehicles.model_id = models.id AND vehicles.country_location = "EE"`
    );
    con.end();
